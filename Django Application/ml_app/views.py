...
# Global variable to store previous confidence scores across requests
previous_confidence_scores = [93, 89, 93, 98, 85, 93, 92, 91, 89, 97]

def predict_page(request):
    global previous_confidence_scores  # Use the global variable to retain previous scores
    ...
    if request.method == "GET":
        ...
        # Perform prediction and collect confidence scores
        try:
            heatmap_images = []
            output = ""
            confidence = 0.0
            ...
            for i in range(len(path_to_videos)):
                ...
                confidence_scores.append(confidence)  # Collect confidence for each frame
                print("Prediction:", prediction[0], "==", output, "Confidence:", confidence)
                print("<=== | Prediction Done | ===>")
                print("--- %s seconds ---" % (time.time() - start_time))

            # Retain previous scores and combine with new scores
            previous_confidence_scores.extend(confidence_scores)

            # Plotting the combined confidence scores
            plt.figure(figsize=(10, 6))
            plt.plot(range(len(previous_confidence_scores)), previous_confidence_scores, marker='o', linestyle='-', color='b')
            plt.title("Accuracy Scores for Each Frame (Updated)")
